#!/usr/bin/env python3
"""Defines a function that creates all masks for training/validation"""
import tensorflow as tf


def create_padding_mask(seq):
    """Creates a padding mask for a batch of token sequences

    Args:
        seq: tf.Tensor of shape (batch_size, seq_len) containing tokens,
            where padding is represented by the value 0

    Returns:
        a tf.Tensor of shape (batch_size, 1, 1, seq_len) containing 1
        wherever seq has a padding token and 0 elsewhere
    """
...
def create_look_ahead_mask(size):
    """Creates a look ahead mask to hide future tokens in a sequence

    Args:
        size: the length of the sequence to mask

    Returns:
        a tf.Tensor of shape (size, size) containing 1 above the main
    ...
    Returns:
        encoder_mask, combined_mask, decoder_mask
            ...
            combined_mask is the tf.Tensor of shape
                ...
                takes the maximum between a look ahead mask and the
